 ...
 *
 * <p>All game logic must retrieve stats exclusively through this class. Constructing
 * {@link EnemyStats}, {@link BossStats}, or {@link PlayerStats} directly inside game
 * code is discouraged — doing so bypasses the registry and breaks the single source
 * of truth that makes the config layer easy to maintain and extend.</p>
 *
 * <p>The backing source is currently the {@link com.lobsterchops.infinitesquareshooter.config.types.EnemyType}
 * and {@link com.lobsterchops.infinitesquareshooter.config.types.BossType} enumerations.
 * Swapping to an external source (JSON file, database, remote config) requires only
 * changes to this class — no game logic needs to be touched.</p>
 *
 * <p>This class is thread-safe. All internal maps are populated once during class
 * initialisation via {@code static} blocks and are never mutated thereafter.</p>
 *
 * <p>Typical usage:</p>
 * <pre>{@code
