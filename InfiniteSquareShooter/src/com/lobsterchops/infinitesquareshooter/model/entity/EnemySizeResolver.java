...
public final class EnemySizeResolver {
	...
	public static float sizeFor(EnemyType type) {
		...
		if (name.startsWith("SWARM")) return 16f;
		if (name.startsWith("TANK")) return 42f;
		if (name.startsWith("BOMBER")) return 36f;
		if (name.startsWith("SPLITTER")) return 34f;
		if (name.startsWith("DASHER")) return 26f;
