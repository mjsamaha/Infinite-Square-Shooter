 ...
 *
 * Mechanics: - Records the player's position in a rolling history buffer (~90
 * entries, ~1.5 s at 60 fps). Each tick the Mimic warps to the oldest buffered
 * entry, creating a shadow that always occupies where the player JUST was. -
 * Does NOT use velocity-based movement; velocity is always ZERO. Position is
 * set directly to the delayed player position each tick once the buffer has
 * warmed up. - Fires toward the player's current position on a per-phase
 * cooldown. - Phase 1 (HP=60): single shot every 800 ms. - Phase 2 (HP=60):
 * spread shot (3 proj, 20° arc) every 600 ms PLUS the phase-1 single shot on
 * its own 800 ms track — "adds on top" of phase-1 pressure with a wider arc and
 * a precise centre shot simultaneously.
 *
 * Visual: - Teal square (72×72). - Phase 1: solid BOSS_MIMIC fill with a single
 * thin darker border. - Phase 2: brighter fill + two inner glow rings to signal
 * the escalation.
 */
public class MimicBoss extends Boss {
	...
	// Phase-2 secondary single-shot track (phase-1 stats reused at their own
	// cadence).
	...
	protected void renderBoss(Graphics2D g2) {
		...
		BufferedImage sprite = SpriteRegistry.forBoss(BossType.MIMIC);
		...
		// Phase-2 glow rings on top of sprite.
		if (phase2) {
			Color glow = new Color(Math.min(255, ColorConfig.BOSS_MIMIC.getRed() + 60),
					Math.min(255, ColorConfig.BOSS_MIMIC.getGreen() + 60),
					Math.min(255, ColorConfig.BOSS_MIMIC.getBlue() + 60), 180);
			g2.setColor(glow);
